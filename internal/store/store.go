...
func (s *Store) ReplayNotification(ctx context.Context, id, callerID string) (Notification, error) {
	tx, err := s.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return Notification{}, fmt.Errorf("begin replay transaction: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	...
	err = tx.QueryRow(ctx, `
		...
		WHERE id = $1 AND caller_id = $2
		FOR UPDATE
	`, id, callerID).Scan(&currentStatus, &attemptCount, &maxAttempts)
	if errors.Is(err, pgx.ErrNoRows) {
		return Notification{}, ErrNotFound
	}
	if err != nil {
		return Notification{}, fmt.Errorf("check notification for replay: %w", err)
	}
	if currentStatus != "dead" {
		return Notification{}, ErrNotDead
	}

	newMaxAttempts := maxAttempts
	if newMaxAttempts < attemptCount+5 {
		newMaxAttempts = attemptCount + 5
	}

	notification, _, err := scanStoredNotification(tx.QueryRow(ctx, `
		...
		SET status = 'pending',
		    ...
		    updated_at = now()
		WHERE id = $1 AND caller_id = $2 AND status = 'dead'
		RETURNING
			id, destination_id, destination_version, status,
			attempt_count, max_attempts, next_attempt_at,
			last_status_code, last_error_code, last_error_message,
			created_at, updated_at, delivered_at, request_hash
	`, id, callerID, newMaxAttempts))
	if err != nil {
		return Notification{}, fmt.Errorf("replay notification: %w", err)
	}

	if err := tx.Commit(ctx); err != nil {
		return Notification{}, fmt.Errorf("commit replay notification: %w", err)
	}
	return notification, nil
}
...
func (s *Store) GetStats(ctx context.Context) (Stats, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT
			...
			COALESCE(COUNT(*) FILTER (WHERE status = 'succeeded'), 0) AS succeeded_count,
			COALESCE(COUNT(*) FILTER (WHERE status = 'dead'), 0) AS dead_count,
	...
	if err != nil {
		return Stats{}, fmt.Errorf("get notification stats: %w", err)
	}
	return stats, nil
}
